...
export class SageMockProvider implements InvoiceProvider, OrderSyncProvider {
  async submitOrder(orderId: string, idempotencyKey: string) {
    if (idempotencyKey.includes("transient-failure"))
      throw new ServiceUnavailableException("Simulated transient Sage failure");
    if (idempotencyKey.includes("permanent-failure"))
      throw new Error("Simulated permanent mapping failure");
    return {
      externalId: `SAGE-MOCK-${orderId.slice(-8).toUpperCase()}`,
