...
import { Channel, FulfilmentStatus, OrderStatus, PaymentStatus, Prisma, UserKind } from "@prisma/client";
...
export class OrdersService {
  constructor(private readonly pricing: PricingService, private readonly prisma: PrismaService, private readonly accounts: AccountsService) {}
  async list() {
  ...
  async create(input: QuoteRequest & { email: string; deliveryAddress: Record<string, string>; notes?: string; purchaseOrder?: string }, key: string, actor?: AuthenticatedUser) {
    ...
    if (input.channel !== "B2C" && (!actor || !input.organizationId)) throw new ConflictException("Authenticated organization context is required");
    const context = input.channel === "B2C" ? null : await this.accounts.assertCanOrder(actor!, input.organizationId!);
    if (context?.organization.poRequired && !input.purchaseOrder?.trim()) throw new ConflictException("A purchase order number is required for this organization");
    ...
    try {
      return await this.prisma.$transaction(async (transaction) => transaction.order.create({
        data: {
          ...
          organizationId: input.organizationId, userId: actor?.id, createdById: actor?.id, agentId: context?.agentId, email: input.email, currency: "GBP", subtotalMinor: quote.subtotal.amount,
          vatMinor: quote.vat.amount, totalMinor: quote.total.amount, billingAddress: input.deliveryAddress,
          deliveryAddress: input.deliveryAddress, notes: input.notes, purchaseOrder: input.purchaseOrder,
  ...
  async createTrade(actor: AuthenticatedUser, input: Omit<QuoteRequest, "channel"> & { email: string; deliveryAddress: Record<string, string>; notes?: string; purchaseOrder?: string }, key: string) {
