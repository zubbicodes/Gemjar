...
type CartItem = Pick<Product, "id" | "variantId" | "name" | "slug" | "sku" | "price" | "image"> & { quantity: number };
  ...
  persist(
    (set) => ({
      ...
      add: (product) => set((state) => {
        ...
        return { items: [...state.items, { id, variantId, name, slug, sku, price, image, quantity: 1 }] };
      }),
