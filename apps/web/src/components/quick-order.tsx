...
type QuoteLine = {
  ...
  gross: Money;
  appliedRule: string;
  validation: { valid: boolean; code?: string; message?: string };
};
type Quote = {
  lines: QuoteLine[];
  subtotal: Money;
...
export function QuickOrder({ kind = "trade" }: { kind?: "trade" | "agent" }) {
  ...
  const [loadedReorder, setLoadedReorder] = useState("");
  const [paymentMethod, setPaymentMethod] = useState<"ON_ACCOUNT" | "CARD">(
    "ON_ACCOUNT",
  );
    ...
    () =>
      new Map(
        catalogue
          ...
          .map((product) => [product.variant!.id, product]),
      ),
    [catalogue],
  );
  ...
  const loadDrafts = useCallback(async (contextId: string) => {
    const body = await readJson(
      await fetch(
        `${API_URL}/carts/drafts?organizationId=${encodeURIComponent(contextId)}`,
  ...
  useEffect(() => {
    ...
    setDraftName("New trade order");
    Promise.all([
      fetch(
        `${API_URL}/trade/products?organizationId=${encodeURIComponent(organizationId)}`,
        { credentials: "include", cache: "no-store" },
      ).then(readJson),
      fetch(
        `${API_URL}/carts/drafts?organizationId=${encodeURIComponent(organizationId)}`,
        { credentials: "include", cache: "no-store" },
      ).then(readJson),
    ])
      .then(([productsBody, draftsBody]) => {
        if (!active) return;
        setCatalogue(productsBody.data);
        setDrafts(draftsBody.data);
      })
      .catch((cause) => {
        if (active)
          setError(
            cause instanceof Error
              ? cause.message
              : "Unable to load the trade catalogue",
          );
      })
      .finally(() => {
  ...
  useEffect(() => {
    if (!reorderId || !organizationId || loadedReorder === reorderId) return;
    void fetch(`${API_URL}/orders/${encodeURIComponent(reorderId)}/reorder`, {
      credentials: "include",
      cache: "no-store",
    })
      .then(readJson)
      .then((body) => {
        if (body.organizationId && body.organizationId !== organizationId)
          throw new Error("This order belongs to another customer context");
        setRows(body.items);
        setDraftName(`Reorder ${reorderId.slice(-8)}`);
        setLoadedReorder(reorderId);
        setMessage(
          "Previous order loaded with current pricing and availability.",
        );
      })
      .catch((cause) =>
        setError(
          cause instanceof Error
            ? cause.message
            : "Unable to load previous order",
        ),
      );
  }, [loadedReorder, organizationId, reorderId]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setQuoting(true);
      setError("");
      void fetch(`${API_URL}/pricing/trade-quote`, {
        method: "POST",
        credentials: "include",
        signal: controller.signal,
        headers: { "Content-Type": "application/json", ...csrfHeaders() },
        body: JSON.stringify({ organizationId, items: rows }),
      })
        .then(readJson)
        .then(setQuote)
        .catch((cause) => {
          ...
          setQuote(null);
          setError(
            cause instanceof Error ? cause.message : "Unable to verify pricing",
          );
        })
        .finally(() => {
  ...
  function addProduct(product: Product) {
    ...
    setRows((current) =>
      current.some((row) => row.variantId === variant.id)
        ? current
        : [
            ...current,
            {
              variantId: variant.id,
              quantity: Math.max(variant.moq, variant.packMultiple),
            },
          ],
    );
    setQuery("");
    ...
    setRows((current) =>
      current.map((row) =>
        row.variantId === variantId
          ? {
              ...row,
              quantity: Math.max(variant.moq, row.quantity + direction * step),
            }
          : row,
      ),
    );
  ...
  function loadDraft(draft: Draft) {
    setRows(
      draft.items.map(({ variantId, quantity }) => ({ variantId, quantity })),
    );
    setDraftId(draft.id);
    setDraftName(draft.name || "Untitled draft");
    setMessage(`Loaded ${draft.name || "draft"}`);
  }
  ...
  async function saveDraft() {
    ...
    try {
      const saved = await readJson(
        await fetch(`${API_URL}/carts/drafts`, {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json", ...csrfHeaders() },
          body: JSON.stringify({
      ...
      await loadDrafts(organizationId);
    } catch (cause) {
      setError(
  ...
  async function submitOrder(event: React.FormEvent<HTMLFormElement>) {
    ...
    setMessage("");
    try {
      const key = orderKey.current ?? crypto.randomUUID();
      ...
      const order = await readJson(
        await fetch(
          `${API_URL}/${paymentMethod === "CARD" ? "payments/trade-checkout" : "orders/trade"}`,
          {
          method: "POST",
          credentials: "include",
          headers: {
      ...
      if (paymentMethod === "CARD") {
        const session = order as CheckoutSession;
        if (session.provider === "mock") {
          const confirmed = await readJson(
            await fetch(
              `${API_URL}/payments/${session.paymentId}/mock-confirm`,
              {
                method: "POST",
          ...
          setMessage(`Card payment confirmed for order ${confirmed.number}.`);
        } else {
          setCardSession(session);
          setMessage("Secure card checkout is ready.");
        }
        return;
      }
      setRows([]);
      ...
      setDraftName("New trade order");
      orderKey.current = null;
      setMessage(`Order ${order.number} was submitted successfully.`);
    } catch (cause) {
      setError(
        cause instanceof Error ? cause.message : "Unable to submit order",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const lineByVariant = new Map(
    quote?.lines.map((line) => [line.variantId, line]) ?? [],
  );

  return (
    <div className="space-y-5">
      <section className="overflow-hidden rounded-[24px] border border-forest/15 bg-forest text-white shadow-soft">
            ...
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[.16em] text-white/55">
                {kind === "agent"
                  ...
                  : "Account pricing context"}
              </p>
              <p className="mt-1 font-display text-2xl font-semibold">
                {selectedOrganization?.name || "Select an approved customer"}
              </p>
            </div>
          </div>
          {organizations.length > 1 && (
            <select
              aria-label="Select customer organization"
              value={organizationId}
              onChange={(event) => setOrganizationId(event.target.value)}
              className="rounded-xl border border-white/15 bg-white/10 px-4 py-3 text-xs font-semibold text-white outline-none focus:border-white/50"
            >
          ...
          {selectedOrganization && (
            <div className="flex gap-2">
              <Badge className="bg-white/10 text-white">
                Net {selectedOrganization.paymentTermsDays}
              </Badge>
      ...
      {error && (
        <div
          role="alert"
          className="flex gap-3 rounded-2xl bg-rose-50 p-4 text-xs text-rose-800"
        >
      ...
      {message && (
        <div
          role="status"
          className="flex gap-3 rounded-2xl bg-emerald-50 p-4 text-xs text-emerald-800"
        >
      ...
      {!loading && !organizations.length ? (
        <section className="surface p-10 text-center">
          <Building2 className="mx-auto size-7 text-ink/25" />
          <h2 className="mt-4 font-display text-2xl font-semibold">
            No approved account available
          </h2>
          <p className="mt-2 text-xs text-ink/45">
            An approved organization or active customer assignment is required
            to build a trade order.
          </p>
        </section>
      ) : (
          ...
          <section className="surface p-5 sm:p-7">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search your catalogue by product or SKU…"
              />
              {matches.length > 0 && (
                <div className="absolute inset-x-0 top-12 z-10 overflow-hidden rounded-2xl border border-ink/10 bg-white shadow-lift">
                  {matches.map((product) => (
                    <button
                      type="button"
                      key={product.id}
                      onClick={() => addProduct(product)}
                      className="flex w-full items-center justify-between border-b border-ink/[.06] px-5 py-4 text-left last:border-0 hover:bg-forest/[.035]"
                    >
                      <span>
                        ...
                        <span className="mt-1 block text-[10px] text-ink/40">
                          {product.variant?.sku} · MOQ {product.variant?.moq} ·
                          Pack {product.variant?.packMultiple}
                        </span>
                      </span>
            ...
            {loading ? (
              <div className="flex items-center justify-center gap-2 py-16 text-xs text-ink/40">
              ...
              <div className="mt-6 overflow-x-auto">
                <table className="w-full min-w-[660px] text-left text-xs">
                  <thead className="text-[10px] uppercase tracking-wider text-ink/35">
                    <tr>
                      <th className="pb-3">Product</th>
                      <th className="pb-3">Verified unit price</th>
                      <th className="pb-3">Quantity</th>
                      <th className="pb-3 text-right">
                        {selectedOrganization?.vatDisplay === "INCLUSIVE"
                          ? "Incl. VAT"
                          : "Ex VAT"}
                      </th>
                      <th />
                  ...
                  <tbody>
                    {rows.map((row) => {
                      ...
                      const variant = product?.variant;
                      const line = lineByVariant.get(row.variantId);
                      if (!product || !variant) return null;
                      return (
                        <tr
                          key={row.variantId}
                          className="border-t border-ink/[.07]"
                        >
                          <td className="py-5">
                            <p className="font-bold">{product.name}</p>
                            <p className="mt-1 text-[10px] text-ink/38">
                              {variant.sku} · MOQ {variant.moq} · Pack{" "}
                              ...
                              <p
                                className={
                                  line.validation.valid
                                    ? "mt-2 text-[10px] text-amber-700"
                                    : "mt-2 text-[10px] font-semibold text-rose-700"
                                }
                              >
                                {line.validation.message}
                              </p>
                          ...
                          <td className="py-5">
                            <p className="font-semibold">
                              {line
                                ? formatMoney(
                                    selectedOrganization?.vatDisplay ===
                                      "INCLUSIVE"
                                      ? Math.round(
                                          line.gross.amount / line.quantity,
                                        )
                            ...
                            {line && (
                              <p className="mt-1 text-[9px] uppercase tracking-wider text-forest/60">
                                {line.appliedRule.replaceAll("_", " ")}
                              </p>
                            )}
                          </td>
                          <td className="py-5">
                            <div className="flex w-fit items-center rounded-full border border-ink/10">
                              <button
                                aria-label={`Decrease ${product.name}`}
                                className="grid size-8 place-items-center"
                                onClick={() =>
                                  changeQuantity(row.variantId, -1)
                                }
                              >
                              ...
                              <button
                                ...
                                className="grid size-8 place-items-center"
                                onClick={() => changeQuantity(row.variantId, 1)}
                              >
                          ...
                          <td className="py-5 text-right font-bold">
                            {line ? formatMoney(line.net.amount) : "—"}
                          </td>
                          <td className="py-5 pl-4">
                            <button
                              aria-label={`Remove ${product.name}`}
                              className="text-ink/30 hover:text-rose-700"
                              onClick={() =>
                                setRows((current) =>
                                  current.filter(
                ...
                {rows.length === 0 && (
                  <div className="py-14 text-center">
                    <Search className="mx-auto size-6 text-ink/20" />
                    <p className="mt-3 text-xs text-ink/40">
                      Search above to add products from this account’s
                      catalogue.
                    </p>
                    <p className="mt-1 text-[10px] text-ink/30">
                      {catalogue.length} product
                      {catalogue.length === 1 ? "" : "s"} available
                    </p>
                  </div>
                )}
              </div>
          ...
          <aside className="space-y-5">
            <section className="surface p-6">
              <div className="flex items-center justify-between">
                <p className="eyebrow">Verified draft</p>
                {quoting && (
              ...
              <input
                ...
                maxLength={100}
              />
              <dl className="mt-5 space-y-3 border-b border-ink/10 pb-5 text-xs">
                <div className="flex justify-between">
                  <dt className="text-ink/45">Subtotal</dt>
                  <dd>{quote ? formatMoney(quote.subtotal.amount) : "—"}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-ink/45">VAT</dt>
                  <dd>{quote ? formatMoney(quote.vat.amount) : "—"}</dd>
                </div>
              </dl>
              <div className="mt-5 flex items-end justify-between">
                <dt className="text-xs font-bold">Total</dt>
                <dd className="font-display text-3xl font-semibold">
                  {quote ? formatMoney(quote.total.amount) : formatMoney(0)}
                </dd>
              </div>
              {quote?.stockConfidence === "PENDING_CONFIRMATION" && (
                <p className="mt-4 rounded-xl bg-amber-50 p-3 text-[10px] leading-4 text-amber-900">
                  Last-known stock is being used. Operations will confirm
                  availability.
                </p>
              )}
              <Button
                ...
                size="lg"
                disabled={
                  !quote || invalid || saving || quoting || !draftName.trim()
                }
              ...
              <p className="mt-4 text-center text-[10px] leading-4 text-ink/38">
                Prices, VAT, pack rules, permissions and current stock are
                verified by the API.
              </p>
            </section>
            <section className="surface p-6">
            ...
            <form className="surface p-6" onSubmit={submitOrder}>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-2 text-xs text-ink/45">
                    Delivery and confirmation details
                  </p>
                </div>
                <Send className="size-5 text-ink/25" />
              </div>
              <div className="mt-5 space-y-3">
                ...
                <label className="block text-xs font-bold">
                  ...
                  <select
                    ...
                    name="paymentMethod"
                    value={paymentMethod}
                    disabled={Boolean(cardSession)}
                    onChange={(event) => {
                      setPaymentMethod(
                        event.target.value as "ON_ACCOUNT" | "CARD",
                      );
                      orderKey.current = null;
                    }}
                  >
                    <option value="ON_ACCOUNT">
                      On account · Net {selectedOrganization?.paymentTermsDays ?? 0}
                    </option>
              ...
              {cardSession &&
                (stripePromise ? (
                  ...
                  <p className="mt-4 rounded-xl bg-rose-50 p-3 text-xs text-rose-800">
                    Stripe checkout requires NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY.
                  </p>
                ))}
            </form>
            <section className="surface p-6">
              <div className="flex items-center justify-between">
                ...
                <FileText className="size-5 text-ink/25" />
              </div>
              <div className="mt-4 space-y-2">
                {drafts.slice(0, 5).map((draft) => (
                  <button
                    ...
                    className={`w-full rounded-xl border p-3 text-left transition ${draft.id === draftId ? "border-forest/30 bg-forest/[.06]" : "border-ink/[.07] hover:border-forest/20"}`}
                  >
                    <span className="block truncate text-xs font-bold">
                      {draft.name || "Untitled draft"}
                    </span>
                    <span className="mt-1 block text-[10px] text-ink/38">
                      {draft.items.length} line
                      {draft.items.length === 1 ? "" : "s"} · v{draft.version} ·{" "}
                ...
                {!drafts.length && (
                  <p className="rounded-xl border border-dashed border-ink/10 p-5 text-center text-[10px] text-ink/35">
  ...
  return (
    <div className="mt-6 border-t border-ink/10 pt-6">
      ...
      >
        ...
        {busy ? "Confirming…" : `Pay ${formatMoney(session.amountMinor)}`}
      </Button>
    </div>
  );
}
