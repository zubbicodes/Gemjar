...
const catalogue = [
  { name: "Verdant Signet", slug: "verdant-signet", sku: "GJ-RNG-042", price: 18900, description: "A sculptural signet in satin gold, set with a deep green lab-grown emerald.", image: "https://images.unsplash.com/photo-1605100804763-247f67b3557e?auto=format&fit=crop&w=1200&q=88", available: 18 },
  { name: "Luna Hoops", slug: "luna-hoops", sku: "GJ-ER-118", price: 9600, description: "Quietly bold hoops with a softly brushed finish and balanced weight.", image: "https://images.unsplash.com/photo-1535632066927-ab7c9ab60908?auto=format&fit=crop&w=1200&q=88", available: 7, moq: 2, packMultiple: 2 },
  { name: "Serein Chain", slug: "serein-chain", sku: "GJ-NK-207", price: 14200, description: "An understated chain with a hand-finished clasp, designed for daily layering.", image: "https://images.unsplash.com/photo-1599643478518-a784e5dc4c8f?auto=format&fit=crop&w=1200&q=88", available: 31 },
  { name: "Solitaire Cuff", slug: "solitaire-cuff", sku: "GJ-BR-076", price: 22500, description: "A precise open cuff punctuated with a bezel-set white sapphire.", image: "https://images.unsplash.com/photo-1611591437281-460bfbe1220a?auto=format&fit=crop&w=1200&q=88", available: 4 },
] as const;
...
async function seedCatalogue() {
  const category = await prisma.category.upsert({ where: { slug: "atelier-edit" }, update: { name: "The Atelier Edit" }, create: { name: "The Atelier Edit", slug: "atelier-edit" } });
  for (const item of catalogue) {
    const product = await prisma.product.upsert({
      where: { slug: item.slug },
      update: { name: item.name, description: item.description, status: ProductStatus.ACTIVE },
      create: { name: item.name, slug: item.slug, description: item.description, status: ProductStatus.ACTIVE, categories: { create: { categoryId: category.id } }, media: { create: { url: item.image, alt: item.name, position: 0 } }, variants: { create: { sku: item.sku, retailPriceMinor: item.price, b2bPriceMinor: Math.round(item.price * .68), moq: "moq" in item ? item.moq : 1, packMultiple: "packMultiple" in item ? item.packMultiple : 1 } } },
      include: { variants: true, media: true },
    });
    const variant = product.variants[0];
    if (variant) {
      await prisma.productVariant.update({ where: { id: variant.id }, data: { retailPriceMinor: item.price, b2bPriceMinor: Math.round(item.price * .68), moq: "moq" in item ? item.moq : 1, packMultiple: "packMultiple" in item ? item.packMultiple : 1 } });
      const stockCount = await prisma.stockSnapshot.count({ where: { variantId: variant.id } });
      if (!stockCount) await prisma.stockSnapshot.create({ data: { variantId: variant.id, available: item.available, capturedAt: item.slug === "luna-hoops" ? new Date(Date.now() - 18 * 60_000) : new Date(), provider: "MINTSOFT_DEMO" } });
    }
    if (!product.media.length) await prisma.productMedia.create({ data: { productId: product.id, url: item.image, alt: item.name, position: 0 } });
  }
}
